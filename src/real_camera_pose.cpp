...
public:
  ...
  : Node("aruco_detector_cpp"), tf_buffer_(this->get_clock()), tf_listener_(tf_buffer_)
  {
    ...
    aruco_params_->cornerRefinementMinAccuracy = 0.3;
    ...
    RCLCPP_INFO(this->get_logger(), "Waiting for tf: base_link -> aruco_link_rotated");
    while (rclcpp::ok()) {
      try {
        cached_marker_tf_ = tf_buffer_.lookupTransform("base_link", "aruco_link_rotated", tf2::TimePointZero);
        break;
