...
public:
  ...
  {
    // Publisher for transform (publishing on /ur_transform topic)
...
private:
  ...
  void publish_transform()
  {
    try
    {
      // Lookup the transform from "base_link" to "rg2_gripper_aruco_link"
      geometry_msgs::msg::TransformStamped transform_stamped;
      transform_stamped = tf_buffer_.lookupTransform("base_link", "rg2_gripper_aruco_link", tf2::TimePointZero);

      // Publish the transform as is, which now corresponds to rg2_gripper_aruco_link.
      transform_publisher_->publish(transform_stamped);

      RCLCPP_INFO(this->get_logger(),
                  "Published rg2_gripper_aruco_link Transform: x=%.3f, y=%.3f, z=%.3f | qx=%.3f, qy=%.3f, qz=%.3f, qw=%.3f",
